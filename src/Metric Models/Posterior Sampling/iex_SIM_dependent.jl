...
function double_iex_multinomial_edit_accept_reject!(
    ...
    γ_curr::Float64,
    mcmc::SimIexInsertDeleteDependent,
    P::CumCondProbMatrix,
...
function double_iex_trans_dim_block_informed_accept_reject!(
    ...
    γ_prop = rand_reflect(γ_curr, ε_td, 0.0, Inf)
    is_insert = (γ_prop > γ_curr)  # Its relative value to current determines whether we insert or delete
...
function (mcmc::SimIexInsertDeleteDependent)(
    posterior::SimPosterior;
    desired_samples::Int=mcmc.desired_samples,
    burn_in::Int=mcmc.burn_in,
    lag::Int=mcmc.lag,
...
function get_td_acc_prob(
    ...
    # Note - insertions or deletions must be at end
    ν_td = mcmc.ν_td
    ...
    aux_mcmc = mcmc.aux_mcmc
    N, M = (length(S_curr), length(S_prop))
    log_ratio = 0.0
    if N < M 
        for i in (N+1):M
            @show i
    ...
    suff_stat_curr = mapreduce(x-> dist(x, S_curr), + , data)
    suff_stat_prop = mapreduce(x -> dist(x, S_prop), + , data)
    log_lik_ratio = (-γ_prop * suff_stat_prop) + (γ_curr * suff_stat_curr)
