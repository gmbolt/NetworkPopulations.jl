...
function print_map_est(output::PosteriorMcmcOutput{T}; top_num::Int=5) where {T<:SisPosterior}
...
function mean_dists_summary(
    chains::Vector{PosteriorMcmcOutput}
)
...
function print_map_est(output::PosteriorMcmcOutput{T}; top_num::Int=5) where {T<:SimPosterior}
